...
from line_detect import line_detection
...
class Tello_Pose:
    def __init__(self):
        ...
        # todo dml_change 6: add new model to recognize line
        self.v1_labels = 'model/googlenet/synset_words.txt'
        self.v1_prototxt = 'model/googlenet/bvlc_googlenet.prototxt'
        self.v1_model = 'model/googlenet/bvlc_googlenet.caffemodel'
        self.v1_net = cv2.dnn.readNetFromCaffe(self.v1_prototxt, self.v1_model)
    ...
    # todo will change 1: two ways to detect circle or line
    ...
    def detect_v1(self, frame):
        ...
        cmd=" "
        rows = open(self.v1_labels).read().strip().split("\n")
        ...
        self.v1_net.setInput(blob)
        start = time.time()
        preds = self.v1_net.forward()
        end = time.time()
        ...
        if classes[idx] == 'water bottle':
            cmd='land'
        print("cll1---",idx,classes[idx],preds[0][idx] * 100 , cmd)
        if cmd==' ':
            cmd=''
        return cmd
